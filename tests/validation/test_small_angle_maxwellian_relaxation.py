"""Maxwellian-relaxation validation for the small-angle collision operator.
...
distributions toward it.  This is the energy-diffusion benchmark used in the
RAMc verification document (Sec. III.B / Fig. 10), generalized here to scan
both background temperature and deliberately different initial distributions.
...
def _print_summary(rows):
    ...
    print(
        "Te[eV]   initial     N      KS(initial)  KS(final)   "
        "<K>/<K>M   <xi>        <xi^2>"
    )
    for row in rows:
        print(
            f"{row['temperature_ev']:7.0f}  "
            f"{row['initial_distribution']:9s}  "
            f"{row['n_markers']:5d}  "
            f"{row['initial_ks_energy']:11.4e}  "
            f"{row['final_ks_energy']:9.4e}  "
            f"{row['final_mean_energy_over_target']:9.5f}  "
            f"{row['final_mean_xi']:10.4e}  "
            f"{row['final_mean_xi2']:9.5f}"
        )
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        nargs="+",
        default=list(DEFAULT_TEMPERATURES_EV),
    )
    ...
    parser.add_argument(
        "--relaxation-thermal",
        type=float,
        default=DEFAULT_RELAXATION_THERMAL,
    )
    ...
    csv_path = args.output_dir / "small_angle_maxwellian_relaxation.csv"
    distribution_path = args.output_dir / "small_angle_maxwellian_relaxation.png"
    summary_path = args.output_dir / "small_angle_maxwellian_summary.png"
    _write_csv(csv_path, rows)
    _write_distribution_plot(distribution_path, rows)
    _write_summary_plot(summary_path, rows)
